...
using VDCBD.API.Helpers;
using VDCBD.API.Interfaces;
using VDCBD.API.Middlewares;
...
builder.Services.AddControllers()
    .ConfigureApiBehaviorOptions(options =>
    {
        options.InvalidModelStateResponseFactory = context =>
        {
            ...
            var response = VDCBD.API.Common.ApiResponse<object>.FailureResponse("Validation failed", errors);
...
using (var scope = app.Services.CreateScope())
{
    ...
    try
    {
        ...
        await VDCBD.API.Data.DbSeeder.SeedAsync(services);
    }
    catch (Exception ex)
    {
